...
<div class="min-h-full flex bg-[#e8f0fe] rounded-lg shadow-lg overflow-hidden mx-auto max-w-sm lg:max-w-3xl">
  <div class="hidden lg:grid lg:w-1/2 bg-cover" style="background-image:url('{{asset('storage/admin-image/login.jpg')}}')"> 
    ...
    <div class="max-w-sm w-full space-y-8 bg-[#e8f0fe] p-6 rounded-md">
      <div>
        <h2 class="mt-6 text-right text-3xl font-extrabold text-primary-800">ورود مدیران</h2>
    ...
    <div>
      ...
      <form  class="mt-8 space-y-4" action="{{route('admin-login-check')}}" method="POST">
        @csrf
      ...
      <div class="rounded-md shadow-sm space-y-0.5">
        <div class="relative">
            <input name="password" value="{{old('password')}}" type="password" id="floating_outlined" class="block border-solid border-2 px-2.5 pb-1.5 pt-2.5 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " />
            <label for="floating_outlined" class="absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-[#e8f0fe] dark:bg-gray-800 px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 right-1">رمز عبور</label>
          </div>
